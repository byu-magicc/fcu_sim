...
#ifndef rotor_gazebo_PLUGINS_GAZEBO_BAG_PLUGIN_H
#define rotor_gazebo_PLUGINS_GAZEBO_BAG_PLUGIN_H
...
#include <rotor_gazebo/AttitudeThrust.h>
#include <rotor_gazebo/default_topics.h>
#include <rotor_gazebo/RateThrust.h>
#include <ros/ros.h>
...
class GazeboBagPlugin : public ModelPlugin {
 ...
 public:
  GazeboBagPlugin()
      : ModelPlugin(),
        ...
        ground_truth_pose_topic_(rotor_gazebo::default_topics::GROUND_TRUTH_POSE),
        ground_truth_twist_topic_(rotor_gazebo::default_topics::GROUND_TRUTH_TWIST),
        imu_topic_(rotor_gazebo::default_topics::IMU),
        control_attitude_thrust_topic_(rotor_gazebo::default_topics::COMMAND_ATTITUDE_THRUST),
        control_motor_speed_topic_(rotor_gazebo::default_topics::COMMAND_ACTUATORS),
        control_rate_thrust_topic_(rotor_gazebo::default_topics::COMMAND_RATE_THRUST),
        motor_topic_(rotor_gazebo::default_topics::MOTOR_MEASUREMENT),
        wrench_topic_(rotor_gazebo::default_topics::WRENCH),
        wind_topic_(rotor_gazebo::default_topics::WIND),
        waypoint_topic_(rotor_gazebo::default_topics::COMMAND_TRAJECTORY),
        command_pose_topic_(rotor_gazebo::default_topics::COMMAND_POSE),
 ...
 protected:
  ...
  /// \param[in] control_msg A AttitudeThrust message from rotor_gazebo.
  void AttitudeThrustCallback(const rotor_gazebo::AttitudeThrustConstPtr& control_msg);

  /// \brief Called when a Actuators message is received.
  /// \param[in] control_msg A Actuators message from rotor_gazebo.
  void ActuatorsCallback(const rotor_gazebo::ActuatorsConstPtr& control_msg);

  /// \brief Called when a RateThrust message is received.
  /// \param[in] control_msg A RateThrust message from rotor_gazebo.
  void RateThrustCallback(const rotor_gazebo::RateThrustConstPtr& control_msg);
...
#endif // rotor_gazebo_PLUGINS_GAZEBO_BAG_PLUGIN_H
