...
void GazeboControllerInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  motor_velocity_reference_pub_ = node_handle_->advertise<rotor_gazebo::Actuators>(motor_velocity_reference_pub_topic_, 10);
}
...
void GazeboControllerInterface::OnUpdate(const common::UpdateInfo& /*_info*/) {
  ...
  rotor_gazebo::ActuatorsPtr turning_velocities_msg(new rotor_gazebo::Actuators);
...
void GazeboControllerInterface::CommandMotorCallback(const rotor_gazebo::ActuatorsConstPtr& input_reference_msg) {
