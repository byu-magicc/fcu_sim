...
#include "rotor_gazebo_plugins/gazebo_bag_plugin.h"
...
void GazeboBagPlugin::AttitudeThrustCallback(
    const rotor_gazebo::AttitudeThrustConstPtr& control_msg) {
  common::Time now = world_->GetSimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
  writeBag(namespace_ + "/" + control_attitude_thrust_topic_, ros_now, control_msg);
}

void GazeboBagPlugin::ActuatorsCallback(const rotor_gazebo::ActuatorsConstPtr& control_msg) {
  common::Time now = world_->GetSimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
  writeBag(namespace_ + "/" + control_motor_speed_topic_, ros_now, control_msg);
}

void GazeboBagPlugin::RateThrustCallback(const rotor_gazebo::RateThrustConstPtr& control_msg) {
  common::Time now = world_->GetSimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
...
void GazeboBagPlugin::LogMotorVelocities(const common::Time now) {
  ...
  rotor_gazebo::Actuators rot_velocities_msg;
