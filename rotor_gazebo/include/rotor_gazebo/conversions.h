...
#ifndef rotor_gazebo_CONVERSIONS_H
#define rotor_gazebo_CONVERSIONS_H
...
#include <rotor_gazebo/eigen_rotor_gazebo.h>
...
#include "rotor_gazebo/TorqueThrust.h"
...
inline void eigenOdometryFromMsg(const nav_msgs::Odometry& msg, EigenOdometry* odometry) {
  ...
  odometry->position_W = rotor_gazebo::vector3FromPointMsg(msg.pose.pose.position);
  odometry->orientation_W_B = rotor_gazebo::quaternionFromMsg(msg.pose.pose.orientation);
  odometry->velocity_B = rotor_gazebo::vector3FromMsg(msg.twist.twist.linear);
  odometry->angular_velocity_B = rotor_gazebo::vector3FromMsg(msg.twist.twist.angular);
}
...
} // end namespace rotor_gazebo

#endif // rotor_gazebo_CONVERSIONS_H
